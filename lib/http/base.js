...
HttpClient.prototype.request = function (options)
{
  ...
  // create a function to send the httpRequest
  var sendRequest = async function sendRequest()
  {
    ...
    // build the basic httpRequest options
    var requestOptions =
      {
    ...
    httpRequest = request(requestOptions.url, requestOptions, async function (err, data, response)
    {
  ...
  // if a httpRequest body is specified and compression is enabled,
  // try to compress the httpRequest body before sending the httpRequest
  ...
  if (body)
  {
    ...
    zlib.gzip(bufferUncompressed, null, function (err, bufferCompressed)
    {
      ...
      {
        Logger.getInstance().warn('Could not compress httpRequest body.');
      }
  ...
  // return an externalized httpRequest object that only contains
...
/**
 * Normalizes a httpRequest headers object so that we get the same behavior
 * regardless of whether we're using httpRequest.js or browser-httpRequest.js.
 *
